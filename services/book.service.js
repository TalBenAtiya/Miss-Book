...
function getGoogleBookAPI(input) {
    const booksArr = storageService.loadFromStorage(input) || null
    if (booksArr) return Promise.resolve(booksArr)

        const books = axios.get(`https://www.googleapis.com/books/v1/volumes?printType=books&q=${input}`)
        return books.then(books => books.data.items).then(infoArr => {
            const newBooks = infoArr.map(book => {
                return {
                    reviews: [],
                    id: book.id,
                    ...
                    listPrice: {
                        amount: utilService.getRandomIntInclusive(30, 180),
                        currencyCode: 'USD',
            ...
            storageService.saveToStorage(input, newBooks)
            return newBooks
        }
        )
    }
